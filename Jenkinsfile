    ...
    stage('Package') {
      steps {
        sh '''
                    ...
                    cpack --debug --verbose                         -G DEB                         -P kstars                         -R $version                         -D CPACK_INSTALL_CMAKE_PROJECTS=".;indi;ALL;/"                         -D CPACK_PACKAGING_INSTALL_PREFIX=/usr/local                         -D CPACK_PACKAGE_FILE_NAME="$package_file_name"                         -D CPACK_PACKAGE_DESCRIPTION_FILE=../.git/HEAD                         -D CPACK_CMAKE_GENERATOR="Unix Makefiles"                         -D CPACK_INSTALL_COMMANDS="make install"                         -D CPACK_PACKAGE_CONTACT="https://github.com/TallFurryMan/kstars-ci"                         -D CPACK_PACKAGE_DESCRIPTION_SUMMARY="INDI Core i386"                         -D CPACK_DEBIAN_PACKAGE_ARCHITECTURE=i386
                    dpkg --info "$package_file_name.deb" || true
                '''
        archiveArtifacts(artifacts: 'indi-build/*.deb', fingerprint: true)
      }
    }
